...
export const getArticles = async (collection_id) => {
  try {
    const results = await AsyncStorage.getItem('@clippy');
    const collections = results != null ? JSON.parse(results) : [];
    const articles = collections.filter(
      (collection) => collection.collection_id === collection_id,
    )[0].data;
    ...
    return sortedArticles[0].type === false
      ? sortedArticles
  ...
  try {
    ...
    const updatedCollections = collections.map((collection) =>
      collection.collection_id === collection_id
        ? {
            ...collection,
            data: collection.data.map((article) =>
  ...
  try {
    ...
    const updatedCollections = collections.map((collection) =>
      collection.collection_id === collection_id
        ? {
            ...collection,
            data: collection.data.filter(
              (article) => article.article_id != selected_article.article_id,
            ),
          }
        : collection,
    );
    const updatedCollection = updatedCollections.map((collection) =>
      collection.collection_id === selected_collection
        ? {
            ...collection,
...
export const deleteArticle = async (data) => {
  ...
  try {
    const results = await AsyncStorage.getItem('@clippy');
    const collections = results != null ? JSON.parse(results) : [];
    const updatedClips = collections.map((collection) =>
      collection.collection_id === collection_id
        ? {
            ...collection,
            data:
